...
$res = $member->getMonthlyExpenseReport($start_date, $end_date);
$grandTotal = 0;
if ($res->num_rows > 0) {
    $index = 0;
    while ($row = $res->fetch_assoc()) {
        ...
        $data .= "<tr>";
        $data .= "<td>" . ++$index . "</td>";
        $data .= "<td><a href='member-expense-monthly-report.php?id=" . $row["member_id"] . "&start=" . $start_date . "&end=" . $end_date. "'>" . $row["username"] . "</a></td>";
        $data .= "<td>" . $row["name"] . "</td>";
